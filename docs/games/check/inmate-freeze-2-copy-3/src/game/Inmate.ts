...
export class Inmate {
    ...
    private size: number;
    private opacity: number = 0.2; // Default 20% opacity
    ...
    private draw(): void {
        ...
        if (this.isFrozen) {
            ...
            this.graphics.beginFill(webColorToHex(GameColors.inmateFrozen), 0.8 * this.opacity);
            this.graphics.lineStyle(2, this.color, 0.8 * this.opacity); // Original color outline
        } else {
        ...
        if (this.isFrozen) {
            ...
            this.graphics.lineStyle(3, webColorToHex(GameColors.inmateTimer), this.opacity);
            this.graphics.arc(0, 0, this.size + 5, 0, progress * Math.PI * 2);
        }
        
        // Draw health indicator if damaged
        if (this.health < this.maxHealth) {
            this.graphics.lineStyle(2, webColorToHex(GameColors.inmateHealthBar), this.opacity);
        ...
        if (!this.isFrozen) {
            ...
            this.graphics.lineStyle(2, webColorToHex(GameColors.inmateDirection), this.opacity);
