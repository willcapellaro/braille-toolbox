...
export class Game {
    ...
    private bonusContainer: Container | null = null;
    private isBonusSelectionActive: boolean = false;
    private availableBonuses: string[] = [
        'Increase spotlight size',
        'Increase battery life', 
        'Repair health 5',
        'Add a policeman',
        'Increase power regeneration',
        'Increase prisoner train length',
        'Increase policeman speed'
    ];
    
    // Bonus effect values (for tracking upgrades)
    private spotlightSizeMultiplier: number = 1.0;
    private maxBatteryMultiplier: number = 1.0;
    private powerRegenMultiplier: number = 1.0;
    private policemanSpeedMultiplier: number = 1.0;
    private prisonerTrainLength: number = 10; // Default max capacity
    ...
    private spawnNewPoliceman(): void {
        // Spawn a new policeman at a random position
        const startX = 400 + (Math.random() * 400); // Random X across middle area
        const startY = 300 + (Math.random() * 100 - 50); // Middle height with variance
        
        const policeman = new Policeman(this.gameContainer, startX, startY);
        this.policemen.push(policeman);
        
        console.log(`New policeman spawned! Total policemen: ${this.policemen.length}`);
    }
    ...
    private updatePolicemen(): void {
        for (const policeman of this.policemen) {
            ...
            if (inmatesForRemoval.length > 0) {
                ...
                // Spawn two new policemen (recruitment mechanic)
                this.spawnNewPoliceman();
                this.spawnNewPoliceman();
    ...
    private showBonusSelection(): void {
        ...
        this.isBonusSelectionActive = true;
        
        // Select 2 random bonuses
        const shuffled = [...this.availableBonuses].sort(() => 0.5 - Math.random());
        const selectedBonuses = shuffled.slice(0, 2);
        
        // Create bonus selection UI
        ...
        // Title text
        const titleText = new Text('CHOOSE YOUR BONUS!', {
            fontFamily: 'Arial',
            fontSize: 36,
            fill: 0xffff00,
            fontWeight: 'bold'
        });
        titleText.x = (this.app.screen.width - titleText.width) / 2;
        titleText.y = 200;
        this.bonusContainer.addChild(titleText);
        ...
        selectedBonuses.forEach((bonus, index) => {
            ...
            buttonBg.beginFill(0x444444, 0.9);
            buttonBg.lineStyle(3, 0xffff00);
            buttonBg.drawRoundedRect(0, 0, 300, 80, 10);
            buttonBg.endFill();
            buttonBg.interactive = true;
            buttonBg.cursor = 'pointer';
            
            // Button text
            const buttonText = new Text(bonus, {
                fontFamily: 'Arial',
                fontSize: 18,
                fill: 0xffffff,
                fontWeight: 'bold',
            ...
            buttonText.x = 10;
            buttonText.y = (80 - buttonText.height) / 2;
            
            buttonContainer.addChild(buttonBg);
            buttonContainer.addChild(buttonText);
            
            // Position buttons
            const buttonX = (this.app.screen.width / 2) + (index === 0 ? -160 : 160) - 150;
            buttonContainer.x = buttonX;
            buttonContainer.y = 300;
            ...
            buttonBg.on('pointerover', () => {
                buttonBg.clear();
                buttonBg.beginFill(0x666666, 0.9);
                buttonBg.lineStyle(3, 0xffff00);
                buttonBg.drawRoundedRect(0, 0, 300, 80, 10);
                buttonBg.endFill();
            });
            ...
            this.bonusContainer.addChild(buttonContainer);
        });
        
        this.app.stage.addChild(this.bonusContainer);
    }
    
    private selectBonus(bonusName: string): void {
        // Apply the selected bonus
        switch (bonusName) {
            case 'Increase spotlight size':
                this.spotlightSizeMultiplier *= 1.3;
                // Apply to existing spotlights
                this.spotlights.forEach(spotlight => spotlight.increaseSize(1.3));
                break;
                
            case 'Increase battery life':
                this.maxBatteryMultiplier *= 1.5;
                this.maxBattery = Math.floor(100 * this.maxBatteryMultiplier);
                this.currentBattery = Math.min(this.currentBattery * 1.2, this.maxBattery);
                break;
                
            case 'Repair health 5':
                this.gameState.lives = Math.min(this.gameState.lives + 5, 35);
                break;
                
            case 'Add a policeman':
                this.spawnNewPoliceman();
                break;
                
            case 'Increase power regeneration':
                this.powerRegenMultiplier *= 1.5;
                this.baseChargeRate *= 1.5;
                break;
                
            case 'Increase prisoner train length':
                this.prisonerTrainLength += 5;
                // Note: Would need to modify Policeman class to use this value
                break;
                
            case 'Increase policeman speed':
                this.policemanSpeedMultiplier *= 1.3;
                // Apply to existing policemen
                this.policemen.forEach(policeman => policeman.increaseSpeed(1.3));
                break;
        }
        
        // Hide bonus selection and resume game
        this.hideBonusSelection();
        
        console.log(`Bonus applied: ${bonusName}`);
    }
    
    private hideBonusSelection(): void {
        if (this.bonusContainer) {
            this.app.stage.removeChild(this.bonusContainer);
            this.bonusContainer.destroy();
            this.bonusContainer = null;
        }
        
        this.isBonusSelectionActive = false;
