  ...
  this.$get = function(ga, $timeout, $rootScope, $document, $window, $q, $modal, $state, StateModal, UpdateCheck) {
    ...
    function restoreImageHistory() {
      ...
      depthy.samples.forEach(function(sample) {
        var imageSource, depthSource, thumb, originalSource;
        
        // Special handling for glassbutt2 sample with custom file names
        if (sample.id === 'glassbutt2') {
          imageSource = 'samples/' + sample.id + '.jpeg';
          depthSource = 'samples/' + sample.id + '-∂map.png';
          thumb = 'samples/' + sample.id + '.jpeg'; // Use image as thumb if no thumb exists
          originalSource = 'samples/' + sample.id + '.jpeg';
        } else {
          // Standard naming convention for other samples
    ...
    depthy = {
      ...
      samples: [
        { id: 'glassbutt2', title: 'Glass Butt 2'},
